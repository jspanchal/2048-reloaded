    ...
    public partial class GamePage
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (bc.Game == null)
            {
                ...
                GameDefinition gameDefinition = games.FirstOrDefault(g => g.UniqueId == (string) e.Parameter);
            ...
            if (TotalScore == 0)
            {
                await PlayGrid.AnimateAsync(new FadeInAnimation {Delay = 0.3});
                await PlayButton.AnimateAsync(new BounceInUpAnimation {Delay = 0.4});
                PlayGrid.IsHitTestVisible = true;
                PlayButton.IsHitTestVisible = true;
            }
            else
...
#endif
            ShareMessage = string.Format("OMG! I scored {0} in #2048Reloaded {1}. (Rank: {2}) @jspanchals {3}", TotalScore, bc.Game.GameDefinition.Title, Rank, uri);
        ...
        private void DataTransferManagerOnDataRequested(DataTransferManager sender, DataRequestedEventArgs args)
        {
            ...
            data.Properties.Title = "Play #2048Reloaded For #WindowsPhone";
            ...
            if (!string.IsNullOrWhiteSpace(ShareMessage))
                ...
                data.SetText(string.Format("I am awesome at #2048Reloaded. High Score: {0} (Rank: {1}) @jspanchals {2}", bc.Game.GameData.BestScore, Rank, uri));
        }
